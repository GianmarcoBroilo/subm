...
simulation_start_epoch = 2459215.50
...
# Define observation simulation times for each link: 3 stellar occultations between 2021-2022
observation_times =  [2459216.50,2459306.93,2459459.50]
...
for epoch in list(variational_equations_solver.state_history):
    ...
    propagated_formal_errors_rsw_dict[epoch] = np.sqrt(np.diag(propagated_covariance_dict[epoch]))
#%%
""""
...
plt.yscale("log")
plt.title("Propagation of $\sigma$ along radial, cross-track and tangential directions")
plt.ylabel('Uncertainty $\sigma$ [m]')
plt.xlabel('Time [s from J2000]')
plt.legend()
plt.show()
