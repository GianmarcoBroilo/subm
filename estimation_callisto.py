...
"""
INPUT
Data type: stellar occultation of Callisto, VLBI Jupiter
...
observations_position = np.arange(simulation_start_epoch,simulation_end_epoch, 100*constants.JULIAN_DAY)
...
apriori_normalized = normalize(covariance_a_priori_inverse, axis=0, norm='l1')
condition_number = np.linalg.multi_dot([pod_output.weighted_normalized_design_matrix.T,pod_output.weighted_normalized_design_matrix]) + apriori_normalized
condition_number = np.linalg.cond(condition_number)
CN = "{:e}".format(condition_number)
print(CN)
condition_correlation = np.linalg.cond(pod_output.correlations)
#%%
...
plt.figure(figsize=(9,5))
plt.plot(time_cal/31536000,values_cal[:,0], label = 'R')
plt.plot(time_cal/31536000,values_cal[:,1], label = 'S')
plt.plot(time_cal/31536000,values_cal[:,2], label = 'W')
